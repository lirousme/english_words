...
<script>
document.querySelectorAll('[data-modal-open]').forEach(button=>button.addEventListener('click',()=>{const modal=document.getElementById(button.dataset.modalOpen);if(button.dataset.id){modal.querySelector('[name="id"]').value=button.dataset.id;modal.querySelector('[name="word"]').value=button.dataset.word;}modal.showModal();modal.querySelector('[name="word"]').focus();}));
let timer;document.getElementById('word-search').addEventListener('input',event=>{clearTimeout(timer);timer=setTimeout(()=>event.target.form.submit(),300);});
function showGeminiAlert(message,type){let alert=document.querySelector('[data-gemini-alert]');if(!alert){alert=document.createElement('div');alert.dataset.geminiAlert='';alert.setAttribute('role','alert');document.querySelector('.words-header').insertAdjacentElement('afterend',alert);}alert.className='alert'+(type==='success'?' alert-success':'');alert.textContent=message;alert.scrollIntoView({block:'nearest',behavior:'smooth'});}
async function refreshTranslations(){const page=await fetch(window.location.href,{headers:{'X-Requested-With':'XMLHttpRequest'}});if(!page.ok)throw new Error('Não foi possível atualizar as traduções exibidas.');const documentAfterUpdate=new DOMParser().parseFromString(await page.text(),'text/html');const updatedCard=documentAfterUpdate.querySelector('.translations-card');const currentCard=document.querySelector('.translations-card');if(updatedCard&&currentCard)currentCard.replaceWith(updatedCard);}
document.addEventListener('submit',async event=>{const form=event.target.closest('.gemini-form');if(!form)return;event.preventDefault();const button=form.querySelector('button[type="submit"]');button.disabled=true;try{const requestBody=new FormData(form);requestBody.append('response_format','json');const response=await fetch(form.action,{method:'POST',body:requestBody,headers:{'Accept':'application/json','X-Requested-With':'XMLHttpRequest'}});const responseText=await response.text();let payload;try{payload=JSON.parse(responseText);}catch{const documentResponse=new DOMParser().parseFromString(responseText,'text/html');const detail=documentResponse.body.textContent.trim();throw new Error(detail?`O servidor retornou uma resposta inesperada: ${detail}`:'O servidor retornou uma resposta vazia em vez de JSON.');}showGeminiAlert(payload.message||'O Gemini retornou uma resposta sem detalhes.',payload.type||'error');if(!response.ok) return;await refreshTranslations();}catch(error){showGeminiAlert(error instanceof Error?error.message:'Não foi possível concluir a solicitação ao Gemini.','error');}finally{button.disabled=false;}});
</script>
<?php appShellFooter();
