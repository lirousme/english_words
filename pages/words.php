...
<section class="words-page">
  <header class="words-header"><div><p class="eyebrow">VOCABULÁRIO</p><h1>Words</h1><p>Gerencie as palavras disponíveis para os seus estudos.</p></div><div class="words-header-actions"><button class="text-button header-action" type="button" data-modal-open="import-csv">Importar CSV</button><form method="post" action="<?= htmlspecialchars(appUrl('words')) ?>" data-async-generation><input type="hidden" name="csrf" value="<?= htmlspecialchars(csrfToken()) ?>"><input type="hidden" name="action" value="generate_audio"><button class="icon-button audio-generate-button" type="submit" aria-label="Gerar áudios das frases sem áudio" title="Gerar áudios pendentes">♫</button></form><button class="icon-button" type="button" data-modal-open="create-word" aria-label="Adicionar palavra">+</button></div></header>
  <div id="words-async-status" aria-live="polite"></div>
...
<dialog class="word-modal" id="manual-translation" aria-labelledby="manual-translation-title"><form method="dialog"><button class="modal-close" aria-label="Fechar">×</button></form><form method="post" action="<?= htmlspecialchars(appUrl('words')) ?>"><input type="hidden" name="csrf" value="<?= htmlspecialchars(csrfToken()) ?>"><input type="hidden" name="action" value="manual_translation"><input type="hidden" name="id"><h2 id="manual-translation-title">Adicionar tradução manualmente</h2><p>Inclua uma tradução e um exemplo nas duas línguas.</p><label>Tradução em português<input name="translation" required maxlength="255"></label><label>Classe gramatical<select name="translation_type" required><?php foreach ($translationTypes as $typeId => $typeName): ?><option value="<?= $typeId ?>"><?= htmlspecialchars($typeName) ?></option><?php endforeach; ?></select></label><label>Frase em inglês<textarea name="english_sentence" required maxlength="2000"></textarea></label><label>Frase em português<textarea name="portuguese_sentence" required maxlength="2000"></textarea></label><button class="button" type="submit">Adicionar tradução e frase</button></form></dialog>
<dialog class="word-modal" id="import-csv" aria-labelledby="import-csv-title"><form method="dialog"><button class="modal-close" aria-label="Fechar">×</button></form><form method="post" enctype="multipart/form-data" action="<?= htmlspecialchars(appUrl('words')) ?>"><input type="hidden" name="csrf" value="<?= htmlspecialchars(csrfToken()) ?>"><input type="hidden" name="action" value="import_csv"><h2 id="import-csv-title">Importar traduções por CSV</h2><p>Sem cabeçalho: palavra, tradução, frase em inglês e frase em português. Use vírgulas ou ponto e vírgula; campos com vírgulas devem estar entre aspas.</p><label>Planilha CSV<input name="csv_file" type="file" accept=".csv,text/csv" required></label><button class="button" type="submit">Importar planilha</button></form></dialog>
<script>document.querySelectorAll('[data-modal-open]').forEach(button=>button.addEventListener('click',()=>{const modal=document.getElementById(button.dataset.modalOpen);if(button.dataset.id){modal.querySelector('[name="id"]').value=button.dataset.id;modal.querySelector('[name="word"]').value=button.dataset.word;}if(button.dataset.wordId)modal.querySelector('[name="id"]').value=button.dataset.wordId;modal.showModal();modal.querySelector('input:not([type="hidden"]), select, textarea')?.focus();}));let timer;document.getElementById('word-search').addEventListener('input',event=>{clearTimeout(timer);timer=setTimeout(()=>event.target.form.submit(),300);});const status=document.getElementById('words-async-status');const setStatus=(message,type='success')=>{status.innerHTML='';const alert=document.createElement('div');alert.className=`alert${type==='success'?' alert-success':''}`;alert.setAttribute('role','alert');alert.textContent=message;status.append(alert);};const refreshTranslations=async()=>{const page=await fetch(window.location.href,{headers:{Accept:'text/html'}});if(!page.ok)throw new Error();const documentUpdate=new DOMParser().parseFromString(await page.text(),'text/html');const current=document.querySelector('.translations-card');const updated=documentUpdate.querySelector('.translations-card');if(current&&updated)current.replaceWith(updated);};document.addEventListener('submit',async event=>{const form=event.target.closest('[data-async-generation]');if(!form)return;event.preventDefault();const button=form.querySelector('button[type="submit"]');button.disabled=true;try{const response=await fetch(form.getAttribute('action'),{method:'POST',headers:{Accept:'application/json'},body:new FormData(form)});const result=await response.json();if(!response.ok||!result.ok)throw new Error(result.message||'Não foi possível concluir a geração.');if(result.ai_response!==undefined)console.info('Resposta da IA:',result.ai_response);setStatus(result.message);if(form.elements.action.value!=='generate_audio')await refreshTranslations();}catch(error){setStatus(error.message||'Não foi possível concluir a geração.','error');}finally{button.disabled=false;}});</script>
<?php appShellFooter();
