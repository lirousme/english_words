...
try {
    ...
    if ($selectedWordId) {
        ...
        if ($selectedWord) {
            $translationStatement = $pdo->prepare('SELECT translations.portugues, translations.`type`, frases.frase_portugues, frases.frase_ingles FROM translations LEFT JOIN frases ON frases.id_translation = translations.id WHERE translations.id_word = :id ORDER BY translations.`type` ASC, translations.portugues ASC');
...
<section class="words-page">
  ...
  <?php if ($selectedWord): ?><section class="translations-card" aria-labelledby="translations-title"><header><div><p class="eyebrow">TRADUÇÕES</p><h2 id="translations-title"><?= htmlspecialchars($selectedWord['word']) ?></h2><p>Traduções armazenadas para esta palavra ou expressão exata.</p></div><form method="post" action="<?= htmlspecialchars(appUrl('words')) ?>"><input type="hidden" name="csrf" value="<?= htmlspecialchars(csrfToken()) ?>"><input type="hidden" name="action" value="discover"><input type="hidden" name="id" value="<?= (int) $selectedWord['id'] ?>"><button class="button" type="submit">Descobrir traduções</button></form></header><?php if ($translations): ?><ul class="translations-list"><?php foreach ($translations as $translation): ?><li><div class="translation-content"><strong><?= htmlspecialchars($translation['portugues']) ?></strong><small><?= htmlspecialchars($translationTypes[(int) $translation['type']] ?? 'Classe desconhecida') ?></small><?php if ($translation['frase_portugues'] && $translation['frase_ingles']): ?><div class="translation-example"><span lang="en"><?= htmlspecialchars($translation['frase_ingles']) ?></span><span><?= htmlspecialchars($translation['frase_portugues']) ?></span></div><?php else: ?><span class="translation-example-missing">Exemplo ainda não disponível. Descubra as traduções novamente para gerá-lo.</span><?php endif; ?></div></li><?php endforeach; ?></ul><?php else: ?><div class="empty-state"><strong>Nenhuma tradução descoberta ainda.</strong><span>Use “Descobrir traduções” para consultar o Gemini.</span></div><?php endif; ?></section><?php elseif ($selectedWordId): ?><div class="alert" role="alert">Palavra não encontrada.</div><?php endif; ?>
