...
try {
    ...
    if ($translation) {
        $statement = $pdo->prepare('SELECT frase_portugues, frase_ingles FROM frases WHERE id_translation = :id ORDER BY id ASC');
  ...
  <?php if ($error): ?><div class="alert" role="alert"><?= htmlspecialchars($error) ?></div><?php elseif ($translation && $sentences): ?>
    <section class="review-card" aria-labelledby="review-title">
      ...
      <div class="review-slides">
        <?php foreach ($sentences as $index => $sentence): ?><article class="review-slide<?= $index === 0 ? ' is-active' : '' ?>" data-slide aria-hidden="<?= $index === 0 ? 'false' : 'true' ?>"><p class="review-portuguese"><?= htmlspecialchars($sentence['frase_portugues']) ?></p><p class="review-english" lang="en"><?= htmlspecialchars($sentence['frase_ingles']) ?></p></article><?php endforeach; ?>
      </div>
    ...
    <script>
      (() => { const slides = [...document.querySelectorAll('[data-slide]')], previous = document.getElementById('previous-slide'), next = document.getElementById('next-slide'), form = document.getElementById('review-form'), counter = document.getElementById('slide-counter'); let current = 0; const render = () => { slides.forEach((slide, index) => { const active = index === current; slide.classList.toggle('is-active', active); slide.setAttribute('aria-hidden', String(!active)); }); previous.disabled = current === 0; counter.textContent = `${current + 1} de ${slides.length}`; const last = current === slides.length - 1; next.textContent = last ? 'Review' : 'Next'; next.type = last ? 'submit' : 'button'; }; previous.addEventListener('click', () => { if (current > 0) { current--; render(); } }); next.addEventListener('click', () => { if (current < slides.length - 1) { current++; render(); } }); form.addEventListener('submit', event => { if (current !== slides.length - 1) event.preventDefault(); }); render(); })();
    </script>
