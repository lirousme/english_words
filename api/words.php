...
function discoverTranslations(string $word): array
{
    ...
    $prompt = "Você é um dicionário inglês-português. Encontre todas as traduções usuais possíveis em português brasileiro da palavra ou expressão inglesa exatamente como fornecida entre <termo> e </termo>.\n\n<termo>{$word}</termo>\n\n"
        ...
        . "Escreva a frase em inglês como um falante nativo a diria em uma conversa. Priorize sempre contrações naturais e comuns em vez das formas expandidas: use, por exemplo, 'I'll' em vez de 'I will', 'I've' em vez de 'I have', 'don't' em vez de 'do not' e 'can't' em vez de 'cannot'. A única exceção é o próprio termo de <termo>, que deve permanecer ipsis litteris para que possa ser estudado.\n"
...
function generateAdditionalSentences(string $word, string $translation, array $existingSentences, int $quantity): array
{
    ...
    $prompt = "Você cria frases de exemplo para um dicionário inglês-português. A palavra ou expressão inglesa original é <termo>{$word}</termo> e a tradução em português brasileiro é <traducao>{$translation}</traducao>.\n\n"
        ...
        . "Escreva cada frase em inglês como um falante nativo a diria em uma conversa. Priorize sempre contrações naturais e comuns: use, por exemplo, 'I'll' em vez de 'I will', 'I've' em vez de 'I have', 'don't' em vez de 'do not' e 'can't' em vez de 'cannot'. A única exceção é o próprio termo de <termo>, que deve permanecer ipsis litteris para estudo. Não repita nem reformule as frases já existentes abaixo: {$existingJson}\n\n"
